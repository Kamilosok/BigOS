...
dt_prop_t* parse_props(const cbuffer_t* fdt_buf, u32 props_offset, u32 props_size, u32 str_offset) {
	...
	while (curr_offset < props_offset + props_size) {
		u32 tag;
		if (!cbuffer_read_u32_be(*fdt_buf, curr_offset, &tag))
			return nullptr;
		...
		if (!cbuffer_read_u32_be(*fdt_buf, curr_offset + 0, &len) ||
		    !cbuffer_read_u32_be(*fdt_buf, curr_offset + 4, &name_offset))
			return nullptr;

		curr_offset += 8;

		const char* name;
		if (!cbuffer_read_cstring(*fdt_buf, str_offset + name_offset, &name))
			return nullptr;
		...
		*new_prop = (dt_prop_t){
		    .name = name,
		    .data = cbuffer_sub_cbuffer(*fdt_buf, curr_offset, len),
		    .next_prop = nullptr,
		};

		if (!cbuffer_is_valid(new_prop->data))
			return nullptr;
...
dt_node_t* parse_subtree(const cbuffer_t* fdt_buf, u32* offset, u32 max_offset, u32 str_offset, dt_node_t* parent) {
	...
	u32 tag;
	if (!cbuffer_read_u32_be(*fdt_buf, curr_offset - 4, &tag) || (fdt_token_t)tag != FDT_BEGIN_NODE)
		return nullptr;

	const char* name;
	if (!cbuffer_read_cstring(*fdt_buf, curr_offset, &name))
		return nullptr;
	...
	while (curr_offset < max_offset) {
		...
		u32 p_len;
		if (!cbuffer_read_u32_be(*fdt_buf, curr_offset + 4, &p_len))
			return nullptr;
